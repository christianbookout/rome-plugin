...
public class ClaimLandCommand implements CommandExecutor, TabCompleter {
    ...
    private boolean removeAllClaims(CommandSender sender, String targetUsername) {
        ...
        return removeAllClaims(sender, target, targetUsername);
    }

    //test if it is possible for the player
    private boolean removeAllClaims(Player player, String targetUsername) {
        var target = SQLConn.getUUIDFromUsername(targetUsername);
        ...
        return removeAllClaims(player, target, targetUsername);
    }
